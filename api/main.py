...
ruta_modelo = "modelo/modelo_recomendacion_pca.pkl"
ruta_titulos = "modelo/titulos.pkl"
ruta_pca = "modelo/pca_model.pkl"
ruta_datos = "datos/data_peliculas.csv"
ruta_datos_reducido = "datos/data_peliculas_reducido.csv"
...
# Función para la cantidad de filmaciones por mes
...
# Función para la cantidad de filmaciones por día
...
# Función para verificar si una película tiene al menos 2000 valoraciones
...
# Función para obtener el éxito de un director basado en el retorno
...
def recomendacion(titulo: str, num_recomendaciones: int = 5):
    if modelo_recomendacion is None or titulos is None or pca is None:
        raise HTTPException(status_code=500, detail="El modelo de recomendación, PCA o los títulos no están cargados.")

    if titulo not in df['titulo'].values:
        raise HTTPException(status_code=404, detail=f"La película '{titulo}' no se encuentra en el catálogo.")
    ...
    try:
        ...
        recomendaciones = [titulos.iloc[i] for i in indices[0][1:num_recomendaciones+1]]
